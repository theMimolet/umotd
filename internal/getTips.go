...
func GetRandomTip(l *gotext.Locale, customTips []string, preset ...string) string {
	...
	if slices.Contains(preset, "gnome") {
		tips = append(tips, []string{
	...
	if slices.Contains(preset, "ublue") {
		tips = append(tips, []string{
			...
			l.Get("`cowsay <something>` makes a cow say something right in your terminal! Install it with `brew install cowsay`"),
		}...)
	}

	if (slices.Contains(preset, "ublue") && slices.Contains(preset, "dev")) || slices.Contains(preset, "ublue-dev") {
		tips = append(tips, []string{
	...
	if slices.Contains(preset, "aurora") {
		tips = append(tips, []string{
	...
	if slices.Contains(preset, "bazzite") {
		tips = append(tips, []string{
	...
	if slices.Contains(preset, "bazzite") && slices.Contains(preset, "gnome") {
		tips = append(tips, []string{
	...
	if slices.Contains(preset, "bluefin") {
		tips = append(tips, []string{
